...
\draw
(chip.west)  ++(-1,\xpos)
node[left] { Pot/CV 1 };\pgfmathsetmacro{\xpos}{ -0.5 }  
\draw[thick, ->] ($(chip.west) + (-1,\xpos)$)
    to[edge label={},out=90,in=90] ++(0.1,0.2)
    to[out=90,in=90] ++(0.1,-0.2)
    -- ($(chip.west) + (-0.5,\xpos)$);
...
\draw
(chip.east)  ++(1,\xpos)
node[right] { Out 0 } ;\pgfmathsetmacro{\xpos}{ -0.5 } 
